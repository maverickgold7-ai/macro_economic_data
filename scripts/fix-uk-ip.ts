...
import { seedCatalog, ensureSchema } from '../src/ingest/pipeline';

async function main() {
  await ensureSchema();
  await seedCatalog();
  const id = 'uk-industrial-production';
  ...
  await client.execute({ sql: 'DELETE FROM observations WHERE metric_id=?', args: [id] });
  for (let i = 0; i < unique.length; i += 300) {
    const chunk = unique.slice(i, i + 300);
    const ph = chunk.map(() => '(?,?,?,?,?,?,1)').join(',');
    const args: (string | number | null)[] = [];
    for (const p of chunk) args.push(id, p.date, p.value, p.rawValue ?? null, now, now.slice(0, 10));
